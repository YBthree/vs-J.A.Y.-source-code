...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'rooblues':
				dialogue = CoolUtil.coolTextFile(Paths.txt('rooblues/roobluesDialogue'));
			case 'feelsgoodman':
				dialogue = CoolUtil.coolTextFile(Paths.txt('feelsgoodman/feelsgoodmanDialogue'));
			case 'ripoff':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ripoff/ripoffDialogue'));
			case 'dontmesswithjay':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dontmesswithjay/dontmesswithjayDialogue'));
		}
			...
			case 'busstop':
				{
					...
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('street/background','weekT'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.8, 0.9);
					bg.active = false;
					add(bg);

					var stageFront:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('street/foreground','weekT'));
					...
					if(songLowercase == 'rooblues'){
						var alexbg:FlxSprite = new FlxSprite(1300, 75);
						alexbg.frames = Paths.getSparrowAtlas('street/alexbgidle','weekT');
						alexbg.animation.addByPrefix('idle', 'alexbg idle', 24);
						alexbg.animation.play('idle');
						alexbg.scrollFactor.set(0.9, 0.9);
						add(alexbg);
					}
					if(songLowercase == 'feelsgoodman'){
						var jinbg:FlxSprite = new FlxSprite(-400, 15);
						jinbg.frames = Paths.getSparrowAtlas('street/jinbgidle','weekT');
						jinbg.animation.addByPrefix('idle', 'jinbg idle', 24);
						jinbg.animation.play('idle');
						jinbg.scrollFactor.set(0.9, 0.9);
						add(jinbg);
					}
					if(songLowercase == 'feelsgoodman'){
						var ybbg:FlxSprite = new FlxSprite(1300, 25);
						ybbg.frames = Paths.getSparrowAtlas('street/ybbgidle','weekT');
						ybbg.animation.addByPrefix('idle', 'ybbg idle', 24);
						ybbg.animation.play('idle');
						ybbg.scrollFactor.set(0.9, 0.9);
						add(ybbg);
						ybbg.flipX = true;
					}
					if(songLowercase == 'rooblues'){
						var ybbg:FlxSprite = new FlxSprite(-200, 25);
						ybbg.frames = Paths.getSparrowAtlas('street/ybbgidle','weekT');
						ybbg.animation.addByPrefix('idle', 'ybbg idle', 24);
						ybbg.animation.play('idle');
						ybbg.scrollFactor.set(0.9, 0.9);
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong == 'dontmesswithjay') 
		{
			switch (curStep)
			{
				case 256:
					remove(dad);
					dad = new Character(0, 100, 'trioa');
					add(dad);
				case 512:
					remove(dad);
					dad = new Character(0, 100, 'trioy');
					add(dad);
				case 812:
					remove(dad);
					dad = new Character(0, 100, 'trio');
					add(dad);
			}
		}
